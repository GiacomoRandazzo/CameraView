 ...
 */
class GlOverlayCameraPreview extends CameraPreview<GLSurfaceView, SurfaceTexture> implements GLSurfaceView.Renderer {

    private boolean mDispatched;
    private final float[] mTransformMatrix = new float[16];
    private int[] mOutputTextureIds;
    private SurfaceTexture mInputSurfaceTexture;
    private EglOverlayViewport mOutputViewport;
    ...
    GlOverlayCameraPreview(@NonNull Context context, @NonNull ViewGroup parent, @Nullable SurfaceCallback callback) {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        mOutputViewport = new EglOverlayViewport();
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        mOutputViewport.drawFrame(mOutputTextureIds, mTransformMatrix);
        for (RendererFrameCallback callback : mRendererFrameCallbacks) {
    ...
    private boolean createBitmap() {
        if (overlayBitmap == null || overlayBitmap.getWidth() != mInputStreamWidth || overlayBitmap.getHeight() != mInputStreamHeight) {
            if (overlayBitmap != null) {
                overlayBitmap.recycle();
            }
            overlayBitmap = Bitmap.createBitmap(mInputStreamWidth, mInputStreamHeight, Bitmap.Config.ARGB_8888);
            return true;
        }
        return false;
    }

    public void drawOverlay(OverlayCanvasDrawer canvasDrawer) {

        if (mOutputViewport == null || mOutputViewport.isOverlayUpdating()) {
            return;
        }

        boolean sizeChanged = createBitmap();
        ...
        mOutputViewport.updateOverlay(overlayBitmap, sizeChanged);
    }
    ...
    void addRendererFrameCallback(@NonNull final RendererFrameCallback callback) {
        getView().queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                if (mOutputTextureIds[0] != 0) callback.onRendererTextureCreated(mOutputTextureIds[0]);
            }
        });
    }
